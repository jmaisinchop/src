...
echo 'Crear Cuenta - Portal Austrobank';
$this->endSection();
?>
...
<div class="container" style="max-width: 550px; margin-top: 4rem; margin-bottom: 4rem;">
    <div class="auth-card">
        ...
        <div class="alert alert-info small">
            ...
            <strong>¿Ya has enviado un ticket antes?</strong> Es posible que ya tengas una cuenta.
            <a href="<?= site_url('portal/activar-cuenta') ?>">Haz clic aquí para recibir tu contraseña.</a>
        </div>

        <?= form_open(site_url('portal/register')) ?>
            <?= csrf_field() ?>
            ...
            <button type="submit" id="submit-btn" class="btn btn-austro btn-block" disabled>Registrarme</button>

            <div class="text-center mt-3">
                <p>¿Ya tienes una cuenta? <a href="<?= site_url('portal/login') ?>">Inicia sesión</a></p>
            </div>
        <?= form_close() ?>
    </div>
</div>
...
$(document).ready(function() {
    ...
    validateBtn.on('click', function() {
        ...
        $.post(captchaValidateUrl, postData, function(response) {
            ...
            if (response.success) {
                feedbackText.text('¡Correcto! Ya puedes registrarte.').removeClass('text-danger').addClass('text-success');
    ...
    // 3. Si el usuario modifica el campo, se debe re-validar
