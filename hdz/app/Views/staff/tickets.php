        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped">
                ...
                <tr>
                    ...
                    <th><?php echo sort_link('department_id_child',lang('Admin.form.dptsAdjunto'));?></th>
                ...
                <?php else:?>
                    <?php foreach ($tickets_result as $item):?>
                        <tr <?php echo isOverdue($item->last_update, $item->status) ? 'class="table-danger"' : '';?>>
                            <td width="15">
                                <input id="checkbox_ticket" type="checkbox" name="ticket_id[]" value="<?php echo $item->id;?>" class="select_item form-check">
                            </td>
                            <td>
                            ...
                            <td>
                                <?php echo getNamesDepAdjuntosById($item->department_id_child); ?>
                            </td>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-4">
                        <div class="form-group">
                            <label><?php echo lang('Admin.form.department');?></label>
                            <select name="department" class="custom-select" id="deparment_list">
                ...
                <div class="form-group">
                    <button id="update_button" class="btn btn-primary"><i class="fa fa-save"></i> <?php echo lang('Admin.form.submit');?></button>
...
?>
<script>
    let allSelect = document.querySelector('#checkbox_ticket');

    $(function (){
        selectTicket();
        ticketsPage();
        $('#select_all, .select_item').change( function (){
            selectTicket(); 
        });
    });

    function selectTicket(){
        let replies = $('#checkbox_ticket').val();
        let selectId = $('.select_item:checked').val();
        let selectLength = $('.select_item:checked').length;
        //let allSelect = document.querySelector('checkbox_ticket');
        if(selectLength > 0){
            console.log('tamaño select: '+selectLength);
        }
        console.log('id ticket: '+selectId);
    }
