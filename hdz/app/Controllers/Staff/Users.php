    ...
    public function editAccount($user_id)
    {
        ...
        if($this->request->getPost('do') == 'submit')
        {
            ...
            if($validation->withRequest($this->request)->run() == false){
                ...
                $msg_update_pwd_staff = "";
                #Obtengo email del Agente
                $dataStaff = findStaffByEmail($this->request->getPost('email')); 
                ...
                if($dataStaff != null){
                    if($this->request->getPost('password')){
                        ...
                        updatePasswordStaff($dataStaff->id, $dataStaff->email, $password);  
                        $msg_update_pwd_staff .= ' / La contraseña del Agente también fue actualizada.'; 
                    }
                }
                $this->session->setFlashdata('form_success', lang('Admin.users.accountUpdated').$msg_update_pwd_staff);
                return redirect()->to(current_url());
            }
        }
        if(defined('HDZDEMO')){
