        ...
        if($this->request->getPost('do') == 'submit'){
            ...
            if($validation->withRequest($this->request)->run() == false){
                ...
                if($this->request->getPost('password')){
                    ...
                    $dataUser = findUserByEmail($this->request->getPost('email'));
                    if ($dataUser != null) {
                        updatePasswordUser($dataUser->id, $dataUser->email, 
                            $this->request->getPost('password'));  
                        $msg_update_pwd_user .= ' / La contraseña del Usuario también fue actualizada.';  
                    }
                }

                $this->session->setFlashdata('form_success',lang('Admin.agents.informationUpdated').$msg_update_pwd_user);
                return redirect()->to(current_url());
            }
        }
    ...
    public function create()
    {
        ...
        if($this->request->getPost('do') == 'submit'){
            ...
            if($validation->withRequest($this->request)->run() == false){
                ...
                /*===================================================================
                =            Para crear usuario con los datos del Agente            =
                ===================================================================*/
                $client = new \App\Libraries\Client();
                $createUser = $this->request->getPost('createUser') ? true : false;

                if($createUser) {
                    $this->client->createAccount($this->request->getPost('fullname'), $this->request->getPost('email'), 
                        $this->request->getPost('password'), true);
                }
                /*=====  End of Para crear usuario con los datos del Agente  ======*/
