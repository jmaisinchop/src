...
class Departments
{
    ...
    public function countTickets($department_id)
    {
        ...
        return $ticketModel->where('department_id', $department_id)
            ->orLike('department_id_child', $department_id)
            ->countAllResults();
    }
    ...
    /**
     * Funcion para contabilizar el numero de departamentos,
     * pasando como parametro el id del departamento padre.
     * */
    public function getCountNumChildDpts($id_dto_padre){
        $total = $this->departmentsModel->groupStart()
                ->Like('id_padre', $id_dto_padre)
                ->groupEnd()
                ->countAllResults();
        return $total;
    }
    ...
    /**
     * Función para obtener los departamentos según el id padre
     * */
    public function getDepartmentsChild($onlyPublic, $id_dto_padre){
        if($onlyPublic){
            $this->departmentsModel->Like('id_padre', $id_dto_padre);
        }
        $q = $this->departmentsModel->orderBy('dep_order','asc')
            ->get();
        if($q->resultID->num_rows == 0){
