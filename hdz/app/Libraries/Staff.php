...
class Staff
{
    ...
    #Función para obtener el registro de Staff, por email
    public function findStaffByEmail($email)
    {
        $q = $this->staffModel->where('email', $email)
        ->get(1);
        if($q->resultID->num_rows == 0){
            return null;
        }
    ...
    #Función para actualizar la contraseña de Staff
    public function updateOnePasswordStaff($id, $email, $password)
    {
        $this->staffModel->protect(false);
    ...
    #Función para buscar Agente por fullname
    public function searchAgent()
    {
        $request = Services::request();
        $per_page = Services::settings()->config('agents_page');
        if($request->getGet('do') == 'search'){
            if($request->getGet('fullname')){
                $fullname = str_replace(['[','#',']'],'', $request->getGet('fullname'));
                $this->staffModel->Like('fullname', $fullname);
            }
        }
    }

    #Process One Way Ticket (Para listar solo Asesores Comerciales)
    public function getAdvisorsCommercial ($filter_commercial)
    {
        $arrayFilter = explode(',', $filter_commercial);
        $not_in = getParamTextArray('NOTIN_COMMERCIAL');
        if(is_array($arrayFilter)) {
            $this->staffModel->groupStart();
            foreach($arrayFilter as $item){
                $this->staffModel->orLike('department', '"'.$item.'"');  
            }
            $this->staffModel->groupEnd();
            $this->staffModel->groupStart();
            foreach($not_in as $id){
                $this->staffModel->notLike('id', $id);
            }
            $q = $this->staffModel->groupEnd()
