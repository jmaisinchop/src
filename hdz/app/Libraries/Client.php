...
class Client
{
    ...
    public function manage()
    {
        $result = $this->usersModel->orderBy('id','desc')
            ->paginate(10, 'default');
        //$search = $this->usersModel->searchUser();
        return [
            'result' => $result,
    ...
    #Función para obtener el registro de Usuario, por email
    public function findClientByEmail($email)
    {
        $q = $this->usersModel->where('email', $email)
        ->get(1);
        if($q->resultID->num_rows == 0){
            return null;
        }
    ...
    public function updateOnePasswordClient($id, $email, $password)
    {
        $this->usersModel->protect(false);
    ...
    #Función para buscar Usuario por fullname
    public function searchUser()
    {
        $request = Services::request();
        $per_page = Services::settings()->config('users_page');
        if($request->getGet('do') == 'search'){
            if($request->getGet('fullname')){
                $fullname = str_replace(['[','#',']'],'', $request->getGet('fullname'));
                $this->usersModel->Like('fullname', $fullname);
            }
        }
    }
    ...
    #METODOS PORTAL EXTERNO
      /***** Cuenta en silencio*/ 
    public function createAccountSilently($name, $email)
    {
        $password = random_string('alnum', 16); // Se autogenera una contraseña segura
        $name = ($name == '' ? $email : $name);
        ...
        return $this->usersModel->getInsertID();
    }

    /**
     * Obtiene el ID de un cliente para el portal. Si no existe,
     * lo crea de forma silenciosa llamando a createAccountSilently().
     */
    public function getClientIDForPortal($name, $email)
    {
        if(!$user_data = $this->getRow(['email' => $email])){
            // Llama al nuevo método de creación silenciosa
            $client_id = $this->createAccountSilently($name, $email);
        }else{
            $client_id = $user_data->id;
        }
        return $client_id;
    }
        public function activateAndSendPassword($client_data)
    {
        // Genera una contraseña aleatoria y legible de 10 caracteres
        $new_password = random_string('alnum', 10);
        
        // Actualiza la contraseña en la base de datos (ya hasheada)
        $this->usersModel->protect(false);
        $this->usersModel->update($client_data->id, [
            'password' => password_hash($new_password, PASSWORD_BCRYPT)
        ]);
        ...
        // Envía el correo usando la nueva plantilla 'account_activation'
        $emails = new Emails();
        $emails->sendFromTemplate('account_activation', [
            '%client_name%'      => $client_data->fullname,
            '%client_email%'     => $client_data->email,
            '%temporary_password%' => $new_password // Enviamos la contraseña en texto plano
        ], $client_data->email);
    }

        public function setupTemporaryPasswordForPortal($client_data)
    {
        $new_password = random_string('alnum', 10);
        
        // Actualiza la contraseña y establece la bandera a 1
        $this->usersModel->protect(false);
        $this->usersModel->update($client_data->id, [
            'password' => password_hash($new_password, PASSWORD_BCRYPT),
        ...
        // Envía el correo usando la plantilla 'account_activation'
        $emails = new Emails();
        $emails->sendFromTemplate('account_activation', [
            '%client_name%'      => $client_data->fullname,
            '%client_email%'     => $client_data->email,
